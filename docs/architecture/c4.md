...
flowchart LR
  ...
  system["ConnectSphere<br/>[Software system]<br/>Coordinates event workflows<br/>Current implementation: foundation only"]
...
flowchart TB
  ...
  subgraph cs["ConnectSphere - software system boundary"]
    web["Web application<br/>[Container: React, TypeScript, Vite]<br/>Runs in browser; displays connectivity status"]
    api["Web server and API<br/>[Container: Python, Flask, SQLAlchemy]<br/>Serves assets, checks DB health, validates identity"]
    db[("Application database<br/>[Container: PostgreSQL]<br/>Current schema: Alembic revision ledger only")]
  end
  auth["Supabase Auth<br/>[External software system]<br/>Issues tokens and validates user identity"]
  user -->|"Views foundation screen"| web
...
The current React shell has no sign-in UI. The real-authentication smoke test obtains a token
from Auth and calls `/api/session` directly; that server path exists today. Browser business
records must go through Flask, never directly to the Supabase Data API. Role checks, organisation
isolation and application-table access restrictions must accompany the first relevant stories.
