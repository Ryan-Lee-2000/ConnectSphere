...
Run focused tests while developing and `npm run verify` before review. Follow the
[test traceability and demonstration convention](testing.md) so a QA case ID locates its documented
expected result and executable assertions. Report any unrun PostgreSQL or browser check. Use
`.github/pull_request_template.md`; link acceptance criteria to test evidence.
